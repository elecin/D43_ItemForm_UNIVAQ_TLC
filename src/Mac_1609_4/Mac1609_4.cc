...
void Mac1609_4::initialize(int stage) {
	...
	if (stage == 0) {
		...
		// Scheduling a self-message to probe the channel in order to get the Channel Busy Ratio if active
		channelProbingActive = par("probeCBR");
		if(channelProbingActive)
		{
            sigCBR = registerSignal("sigCBR");

		    simtime_t samplingStartDelay;
		    samplingStartDelay = simTime()+0.05+uniform(-0.05, 0.05);

            probeNumber = 0;
            probeIntervalStart = floor(samplingStartDelay.dbl());
            busyChannelCount = 0;
            nextChannelProbe = new cMessage("ChannelProbe");
            scheduleAt(samplingStartDelay, nextChannelProbe);
		}
	}
}
...
void Mac1609_4::handleSelfMsg(cMessage* msg) {
	...
	else if (msg ==  nextChannelProbe)
	{
	    if(simTime().dbl() < probeIntervalStart+1)
	    {
	        probeNumber++;
	        if(!idleChannel)
            {
	    ...
	    {
	        // Compute the interval CBR deci-percent
	        double cbr;
	        cbr=0;
	        if(probeNumber != 0)
	        {
	            cbr=(busyChannelCount/probeNumber)*10;
	            emit(sigCBR, cbr);
	        }
	        else
	        {
	        ...
	        // Var update
	        probeIntervalStart = floor(simTime().dbl());
	        probeNumber=0;
	        busyChannelCount=0;

            // Get the new measures set
            if(!idleChannel)
            {
                busyChannelCount++;
            }
	    ...
	    scheduleAt(simTime()+0.00001+uniform(-0.000005, 0.000005), nextChannelProbe);
	}
}
